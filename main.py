...
def total_bills():
  ...
  # ============Drinks Item quantity ===================
        ...
        # ============= Foods Item quantity ======================
        roti_q = roti_qty.get()
        Alu_bhorta_q = Alu_bhorta_qty.get()
        ...
        if Alu_bhorta_var.get() == 0:
                Alu_bhorta_q = 0
        elif Alu_bhorta_var.get() == 1 and Alu_bhorta_qty.get() == "":
                messagebox.showerror("error","please fill the Alu Bhorta quantity")
                coffee_q = 0
        ...
        total_Alu_bhorta_price = Alu_bhorta_price * int(Alu_bhorta_q)
        ...
        total_rice_price = rice_price * int(rice_q)
         ...
         # ============ Total Foods cost ===================
        total_foods_cost = total_roti_price + total_Alu_bhorta_price + total_mutter_panner_price + total_paratha_price + total_mix_veg_price + total_omelete_price + total_veg_biryani_price + total_rice_price
...
def Alu_bhorta_chk():
        if Alu_bhorta_var.get() == 1:
                Alu_bhorta_qty['state'] = "normal"
                Alu_bhorta_qty['bg'] = '#248aa2'
                Alu_bhorta_qty['fg'] = "white"
        else:
               Alu_bhorta_qty['state'] = "disabled"
...
def get_order_details():
    # Gather order details from your UI
    # For this example, let's assume you have variables like drinks_cost, foods_cost, and total_cost
    drinks_cost_value = drinks_cost.get()
    foods_cost_value = foods_cost.get()
    total_cost_value = total_cost_cost.get()
    
    # Create a list containing the order details
    order_details = [['Drinks Cost', drinks_cost_value],['Foods Cost', foods_cost_value],
                    ['Total Cost', total_cost_value]]
    
    return order_details

# Function to write order details to CSV file
def write_order_to_csv(order_details):
    ...
    with open(csv_file, mode='w', newline='') as file:
        ...
        writer.writerow(['Item', 'Cost'])  # Write header
        writer.writerows(order_details)

# After the user confirms their order, call these functions
...
Alu_bhorta_var = IntVar()
Alu_bhorta = Checkbutton(foods,text="Alu Bhorta",variable=Alu_bhorta_var,font=('verdana',8,'bold'),command=Alu_bhorta_chk)
Alu_bhorta.place(x=2,y=22)
Alu_bhorta_qty = Entry(foods,width=15,borderwidth=4,relief=SUNKEN,state="disabled")
Alu_bhorta_qty.place(x=140,y=22)
